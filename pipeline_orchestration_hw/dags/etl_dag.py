...
def load_data():
    ...
    api.dataset_download_file(
        ...
        "/opt/airflow/data/twitter_training.csv",
    )
    api.dataset_download_file(
        "jp797498e/twitter-entity-sentiment-analysis",
        "/opt/airflow/data/twitter_validation.csv",
    )
...
def process_data():
    ...
    df_train[["neg_vader_score", "neu_vader_score", "pos_vader_score"]] = df_train[
        "text"
    ].apply(lambda x: sentiment_scores(x))
    df_test[["neg_vader_score", "neu_vader_score", "pos_vader_score"]] = df_test[
        "text"
    ].apply(lambda x: sentiment_scores(x))
...
with dag:
    load_data_task = PythonOperator(task_id="clean_text", python_callable=load_data)
